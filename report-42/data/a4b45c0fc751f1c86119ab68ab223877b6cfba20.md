...
Locator: locator('[data-testid="descripcion-transaccion"]').first()
Expected string: "email1751563631564@valido.com"
Received string: "Transferencia de email1752334386735@valido.com"
Call log:
  - expect.toContainText with timeout 5000ms
  - waiting for locator('[data-testid="descripcion-transaccion"]').first()
    9 × locator resolved to <p data-testid="descripcion-transaccion" class="MuiTypography-root MuiTypography-body1 css-wl91uy-MuiTypography-root">Transferencia de email1752334386735@valido.com</p>
      - unexpected value "Transferencia de email1752334386735@valido.com"

    at /home/runner/work/taller-playwright-atenea/taller-playwright-atenea/tests/transacciones.spec.ts:96:69
```
...
- heading "$1,107.00" [level=3]
- button "Congelar"
- heading "$1107.00" [level=6]
- heading "•••• 2203" [level=6]
- paragraph: Débito
- text: +
...
- heading "12/7/2025" [level=6]
- paragraph: Transferencia de email1752334386735@valido.com
- paragraph: + 7.00
- paragraph: Transferencia de email1752334386735@valido.com
- paragraph: + 100.00
- paragraph: Cuenta activada 2203
- paragraph: + 1000.00
```
...
```ts
   ...
   38 | })
   39 |
   40 | // Test unificado que envía dinero por API y verifica en la UI.
   ...
   66 |         }
   67 |     });
   68 |     expect(respuestaDeCuentas.ok(), `La API para obtener cuentas falló: ${respuestaDeCuentas.status()}`).toBeTruthy();
   ...
   85 |         }
   86 |     });
   87 |     expect(respuestaDeTransferencia.ok(), `La API para transferir dinero falló: ${respuestaDeTransferencia.status()}`).toBeTruthy();
   88 |
...
index 9a47468..9e70979 100644
--- a/tests/registro.setup.ts
+++ b/tests/registro.setup.ts
@@ -4,8 +4,6 @@ import TestData from '../data/testData.json';
...
@@ -13,7 +11,6 @@ let modalCrearCuenta: ModalCrearCuenta;
...
@@ -24,10 +21,6 @@ setup.beforeEach(async ({ page }) => {
...
@@ -37,10 +30,15 @@ setup('Generar usuario que envía dinero', async ({ page, request }) => {
